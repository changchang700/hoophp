...
class LibPage {
    ...
    public function show() {
        return str_ireplace(array('{total}', '{pagesize}', '{start}', '{end}', '{pagenum}', '{frist}', '{pre}', '{next}', '{last}', '{list}', '{goto}',), array($this->total, $this->setPageSize(), $this->star(), $this->end(), $this->pageNum, $this->frist(), $this->prev(), $this->next(), $this->last(), $this->pagelist(), $this->gopage(),), $this->template);
    }
    /**
     * 获取limit起始数
     * @return type
    ...
    private function setPage() {
        if (!empty($_GET[$this->pageParam])) {
            if ($_GET[$this->pageParam] > 0) {
                if ($_GET[$this->pageParam] > $this->pageNum) return $this->pageNum;
                else return $_GET[$this->pageParam];
            }
        }
        return 1;
    }
